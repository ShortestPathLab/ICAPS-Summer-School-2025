...
pyTaskScheduler::pyTaskScheduler(SharedEnvironment *env) : TaskScheduler(env) {
  ...
  // default
  std::cout << "setting to default python path: ./python, ../python, ./build, "
               "/MAPF/codes/build "
            << std::endl;
  ...
  sys.attr("path").attr("append")("/MAPF/codes/build");
  ...
  if (configFile) {
    ...
    try {
      ...
      if (configData.contains("python_path")) {
        ...
        std::cout << "addinng " << python_path << " to system path"
                  << std::endl;
...
void pyTaskScheduler::plan(int time_limit,
  ...
  try {
    ...
    std::cerr << "Error: Failed to cast python object to c++ object. "
              << e.what() << std::endl;
  } catch (const pybind11::error_already_set &e) {
    std::cerr << "Error: Failed to call python function. " << e.what()
              << std::endl;
